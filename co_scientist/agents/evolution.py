...
class EvolutionAgent(BaseAgent):
    ...
    async def _unary(
        ...
        h = top[0]
        review = await review_bundle(self.deps, h.id)
        ...
        search_space_item = await self._search_space_item(h)
        if search_space_item:
            prompt += (
                "\n\nCONSTRAINED SEARCH-SPACE ITEM: "
                f"{search_space_item}\nRefine only the proposal for this exact "
                "candidate. Do not replace, combine, or rename the candidate."
            )
        return await self._run_and_persist(
            session, prompt, strategy=strategy,
            mode_for_route=strategy,
            parent_ids=[h.id],
            search_space_item=search_space_item,
        )
    ...
    async def _run_and_persist(
        ...
        *,
        ...
        mode_for_route: str,
        parent_ids: list[str],
        search_space_item: str | None = None,
    ) -> str | None:
        sys_blocks = [
        ...
        if search_space_item:
            record["search_space_item"] = search_space_item
    ...
    async def _search_space_item(self, hypothesis: Hypothesis) -> str | None:
        try:
            artifact = await read_json(self.deps.cfg, hypothesis.artifact_path)
        except Exception:
            return None
        record = artifact.get("record") if isinstance(artifact, dict) else None
        if not isinstance(record, dict):
            return None
        item = record.get("search_space_item")
