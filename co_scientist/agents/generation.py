"""Generation agent — proposes new hypotheses via literature or debate."""
...
class GenerationAgent(BaseAgent):
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        search_space_item = task.payload.get("search_space_item")
        ...
        if strategy not in ("literature", "debate"):
            raise NotImplementedError(f"strategy {strategy!r} lands in a later milestone")

        if strategy == "literature":
            # 1. Render the prompt and run the tool loop with structured output.
            articles_block = (
                ...
                "(no hits) is evidence that the literature you searched for may not "
                "exist. After at most 2-3 focused searches with no relevant hits, "
                "record the candidate and let Reflection perform the independent "
                "novelty check."
            )
            prompt = render(
                ...
                instructions=(
                    "Propose ONE hypothesis and register it via record_hypothesis. "
                    "Additional hypotheses come from separate Generation calls. "
                    "Budget literature search to a handful of focused queries."
                    + (
                        "\n\nCONSTRAINED SEARCH-SPACE ITEM: "
                        f"{search_space_item}\nEvaluate this exact candidate. "
                        "Do not replace it with a different candidate. If evidence "
                        "argues against it, record that uncertainty honestly while "
                        "still producing the candidate-specific hypothesis."
                        if search_space_item
                        else ""
                    )
                ),
            )
        else:
            debate_transcript = await self._run_debate_turns(
                session,
                reviews_overview=await _reviews_overview(self.deps, session.id),
            )
            prompt = render(
                "generation.debate",
                goal=plan.objective,
                ...
                instructions=(
                    "The transcript below was produced by real alternating model "
                    "calls. Synthesize it into exactly ONE refined hypothesis and "
                    "register it via record_hypothesis."
                    + (
                        "\n\nCONSTRAINED SEARCH-SPACE ITEM: "
                        f"{search_space_item}\nThe final hypothesis must evaluate "
                        "this exact candidate and may not substitute another one."
                        if search_space_item
                        else ""
                    )
                ),
        ...
        r = route(self.deps.cfg, "generation", strategy)
        ...
        ctx = CallContext(
            ...
            agent="generation", action="CreateInitialHypotheses", mode=strategy,
        )
        ...
        if search_space_item:
            record["search_space_item"] = str(search_space_item)
        hid, was_new = await self._persist(
            session.id,
            record,
            strategy=strategy,
            allow_semantic_dedup=not bool(search_space_item),
        )
        return TaskResult(
    ...
    async def _run_debate_turns(self, session, *, reviews_overview: str) -> str:
        """Run an alternating visionary/skeptic exchange across separate calls."""
        turns: list[str] = []
        n_turns = max(2, self.deps.cfg.run.generation_debate_turns)
        for turn in range(n_turns):
            visionary = turn % 2 == 0
            role = "Visionary" if visionary else "Skeptic"
            instruction = (
                "Propose or materially refine one bold, specific mechanism. "
                "Address the strongest criticism from earlier turns."
                if visionary
                else
                "Attack unsupported assumptions, identify contradictory evidence, "
                "and demand one decisive falsification experiment."
            )
            prompt = (
                f"You are the {role} in turn {turn + 1} of an alternating "
                "scientific self-play debate.\n\n"
                f"Research goal: {session.research_plan.objective}\n"
                f"Criteria: {'; '.join(session.research_plan.preferences)}\n\n"
                f"Prior reviewed ideas:\n{reviews_overview}\n\n"
                "Debate so far:\n"
                + ("\n\n".join(turns) if turns else "(none)")
                + f"\n\nYour task: {instruction}\n"
                "Respond only with this turn. Do not call tools and do not finalize "
                "a structured record."
            )
            spec = AgentCallSpec(
                route=route(self.deps.cfg, "generation", "debate"),
                system_blocks=[
                    CachedBlock(self._system_prompt_header(), cache=True),
                    CachedBlock(
                        _build_session_context(
                            ...
                            await _latest_system_feedback(
                                self.deps, session.id
                            ),
                        ),
                        cache=True,
                    ),
            ...
            response = await self.deps.llm.call(
                ...
                CallContext(
                    ...
                    task_id=None,
                    agent="generation",
                    action="ScientificDebateTurn",
                    mode="debate",
                ),
            )
            text = self._final_text(response)
            turns.append(f"Turn {turn + 1} - {role}:\n{text}")
        return "\n\n".join(turns)
    ...
    async def _persist(
        ...
        *,
        strategy: str,
        allow_semantic_dedup: bool = True,
    ) -> tuple[str, bool]:
        ...
        if allow_semantic_dedup:
            ...
            # A bounded search-space treats every assigned entity as an
            # independent experimental unit. Similar mechanisms must not merge
            # distinct candidates.
            dup_id, embed_payload = None, None
...
async def _reviews_overview(deps: AgentDeps, session_id: str) -> str:
    """Compact prior results into a debate seed without replaying full transcripts."""
    from ..storage.repos import reviews as rev_repo

    hypotheses = await hyp_repo.list_for_session(deps.db, session_id)
    reviews = await rev_repo.list_for_session(deps.db, session_id)
    reviews_by_hyp = {review.hypothesis_id: review for review in reviews}
    lines: list[str] = []
    for hypothesis in hypotheses[:8]:
        review = reviews_by_hyp.get(hypothesis.id)
        review_text = (
            f"verdict={review.verdict}; {review.body[:600]}"
            if review is not None
            else "not reviewed"
        )
        lines.append(
            f"- {hypothesis.title}: {hypothesis.summary[:500]}\n  {review_text}"
        )
    return "\n".join(lines) or "(no prior hypotheses available)"
