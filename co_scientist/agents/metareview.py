...
class MetaReviewAgent(BaseAgent):
    ...
    async def _final_overview(self, task: Task) -> TaskResult:
        ...
        rationales = await tourney_repo.recent_rationales(
            self.deps.db, session.id, limit=100
        )
        rationale_block = "\n\n".join(
            rationale[:2000] for rationale in rationales if rationale
        )

        # Build the top-hypotheses block with the complete evidence-aware reviews.
        chunks: list[str] = []
        for h in top:
            reviews = await review_bundle(self.deps, h.id)
