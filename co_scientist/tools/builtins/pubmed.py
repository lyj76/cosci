...
class PubmedSearchTool:
    ...
    input_schema: dict[str, Any] = {
        "type": "object",
        "properties": {
            "query": {"type": "string", "description": "PubMed query (E-utilities syntax allowed)."},
            "max_results": {
    ...
    async def call(self, args: dict[str, Any], ctx: ToolCtx) -> ToolResult:
        ...
        api_key = (
            self._cfg.secrets.NCBI_API_KEY or os.environ.get("NCBI_API_KEY") or None
        )
    ...
    async def _esearch(
        ...
        r = await self._rate_limited_get(client, ESEARCH, params, api_key)
        r.raise_for_status()
        data = r.json()
    ...
    async def _efetch(
        ...
        if api_key:
            params["api_key"] = api_key
        r = await self._rate_limited_get(client, EFETCH, params, api_key)
        r.raise_for_status()
    ...
    async def _rate_limited_get(
        ...
        # NCBI permits 3 req/s without a key and 10 req/s with one.
