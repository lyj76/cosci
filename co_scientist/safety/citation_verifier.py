...
class CitationVerifier:
    ...
    async def verify_review(
        ...
        return await self.verify_evidence(
    ...
    async def verify_evidence(
        ...
        *,
        ...
        """Fetch and verify evidence excerpts, emitting audit events on failures."""
        if not self._cfg.safety.enable_citation_verifier or not evidence:
            return {}
        session = await sess_repo.fetch(db, session_id)
        if session is None:
            return {}

        ctx = ToolCtx(cfg=self._cfg, session_id=session_id, task_id=None, run_id=review_id)
        out: dict[str, Any] = {}
        for index, ev in enumerate(evidence):
            ...
            if result.is_error or not isinstance(result.content, dict):
                out[key] = {"status": "fetch_failed", "error": result.error_message}
                continue
        ...
        if failures:
            await events_repo.emit(
                ...
                payload={"review_id": review_id, "failures": failures},
            )
        return out


def summarize_verification(results: dict[str, Any]) -> dict[str, Any]:
    counts = {"ok": 0, "unverified": 0, "fetch_failed": 0}
    ...
    return {
        **counts,
        "total": total,
        "verified_fraction": counts["ok"] / total if total else None,
    }
