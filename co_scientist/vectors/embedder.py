...
class OpenAIEmbedder:
    ...
    async def embed(self, texts: list[str]) -> np.ndarray:
        ...
        # DashScope text-embedding-v3/v4 accepts at most 10 strings per call.
        batch_size = 10 if self._base_url and "dashscope.aliyuncs.com" in self._base_url else 256
...
def make_embedder(cfg: Config) -> Embedder:
    ...
    if provider == "voyage":
        ...
        openai_key = cfg.secrets.OPENAI_API_KEY or os.environ.get("OPENAI_API_KEY")
        base_url = (
            getattr(cfg.llm.openai, "base_url", None)
            or os.environ.get("OPENAI_BASE_URL")
            or ""
        )
        custom_openai_endpoint = bool(base_url) and "api.openai.com" not in base_url.lower()
        if openai_key and not custom_openai_endpoint:
            _warn_once("voyage_key_missing_using_openai_embeddings")
            return OpenAIEmbedder(cfg)
        if openai_key and custom_openai_endpoint:
            _warn_once("voyage_key_missing_using_hash_fallback")
