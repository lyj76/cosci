...
def test_paper_aml_preset_bundles_goal_and_goldset() -> None:
    ...
    # The paper-derived candidate-specific evaluation criteria should be present.
    g = p.default_goal.lower()
    assert "prior" in g and "preclinical" in g
    assert "approved indication" in g
    assert "rank" in g
...
def test_deepseek_uplift_preset_is_same_model_ablation() -> None:
    p = get_preset("deepseek-aml-uplift")
    assert {candidate.mode for candidate in p.candidates} == {
        "session",
        "pipeline",
        "direct",
    }
    assert {candidate.model for candidate in p.candidates} == {"deepseek-v4-pro"}
    assert {candidate.provider for candidate in p.candidates} == {"openai_compatible"}
    assert p.goldset is not None
...
def test_session_gold_scoring_uses_only_internal_top_k() -> None:
    state = _CandidateState(
        candidate_id="session",
        spec=BenchCandidate("session", "openai_compatible", "model", mode="session"),
    )
    low = _make_hyp("low", "contains KIRA6")
    low.elo = 1100
    high = _make_hyp("high", "unrelated")
    high.elo = 1300
    state.hypotheses = [low, high]
    state.hypothesis_records = [
        {"id": low.id, "title": "KIRA6"},
        {"id": high.id, "title": "Higher-ranked candidate"},
    ]

    assert _records_for_gold_scoring(state, top_k=1) == [
        {"id": high.id, "title": "Higher-ranked candidate"}
    ]
...
async def test_cross_tournament_runs_n_matches_per_pair(conn) -> None:
    ...
    for lbl in ["A", "B", "C"]:
        st = _CandidateState(
            candidate_id=f"bcd_{lbl}", spec=BenchCandidate(lbl, "openrouter", f"model-{lbl}")
        )
...
async def test_cross_tournament_handles_empty_candidate(conn) -> None:
    ...
    good = _CandidateState(candidate_id="bcd_good", spec=BenchCandidate("good", "openrouter", "m"))
    h = _make_hyp("hyp_g")
    good.hypotheses = [h]
    good.elos = {h.id: 1200.0}
    good.matches_played = {h.id: 0}

    bad = _CandidateState(candidate_id="bcd_bad", spec=BenchCandidate("bad", "openrouter", "m"))
    ...
    with patch(
        "co_scientist.bench.runner._judge_match", new=AsyncMock(return_value=("a", "ok", 0.0, 10))
    ):
        n = await _run_cross_tournament(
            conn,
            "bnc_t2",
